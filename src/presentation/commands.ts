...
export class CommandHandler {
  ...
  private async syncProviderModels(providerId: string, source: 'manual' | 'auto'): Promise<void> {
    ...
    try {
        ...
        async (_progress, _token) => {
          ...
          for (const remote of remoteModels) {
            ...
            if (remoteCapabilities.toolCalling === undefined) {
              remoteCapabilities.toolCalling = true;
            }
            const model: Model = {
      ...
      if (result.totalRemote === 0) {
        ...
        UserFeedback.showWarning(message);
        logger.warn('syncProviderModels empty result', {
          provider: logger.sanitizeProvider(fetchableProvider),
        });
        return;
      }
  ...
  async deleteModels(items: ModelTreeItem[]): Promise<void> {
    ...
    if (ConfigManager.getConfirmDelete()) {
      ...
      if (selection !== deleteOption && selection !== deleteDontAskOption) {
        logger.debug('deleteModels canceled by user', { count });
        return;
      }
    }
  ...
  async exportConfig(): Promise<void> {
    ...
    try {
      ...
      const passwordInput = await UserFeedback.showInputBox({
        prompt:
          'Enter password to encrypt (API Keys included). Leave empty for plain JSON (API Keys EXCLUDED).',
      ...
      await UserFeedback.showProgress('Exporting configuration...', async (_progress, _token) => {
        ...
        UserFeedback.showInfo(
          `Configuration exported${password ? ' (encrypted)' : ''} to ${targetUri.fsPath}`
        );
      ...
      logger.error('exportConfig failed', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
    ...
    try {
      ...
      await UserFeedback.showProgress('Importing configuration...', async (_progress, _token) => {
        ...
        for (const provider of providers) {
          ...
          for (const model of provider.models) {
            ...
            if (
              ...
              !mm['family'] ||
              !mm['version'] ||
              typeof mm['maxInputTokens'] !== 'number' ||
              typeof mm['maxOutputTokens'] !== 'number'
            ) {
              throw new Error('Configuration format is invalid');
            }

            const capabilitiesValue = mm['capabilities'];
            const hasCapabilitiesObject =
              typeof capabilitiesValue === 'object' && capabilitiesValue !== null;
        ...
        if (currentProviders.length > 0) {
          const overwrite = await UserFeedback.showConfirmDialog(
            'Current configuration already exists, do you want to overwrite it?'
          );
      ...
      logger.error('importConfig failed', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
