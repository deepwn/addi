...
export class ModelCommandHandler extends BaseCommandHandler {
  ...
  async deleteModels(items: ModelTreeItem[]): Promise<void> {
    ...
    if (!firstItem) return;
    const providerName = firstItem.providerName;
    const count = items.length;
    const message =
      count === 1 && firstItem
        ? vscode.l10n.t('Are you sure you want to delete the model "{0}"?', firstItem.model.id)
    ...
    try {
      await this.manager.deleteModels(
        providerName,
        items.map((i) => i.model.id),
      );
  ...
  async selectModel(item: ModelTreeItem): Promise<void> {
    if (!item.model.toolCalling) {
      vscode.window.showWarningMessage(
        vscode.l10n.t(
          'Model "{0}" does not support tool calling and cannot be used in Copilot.',
    ...
    try {
      ...
      logger.info(
        "Chat model selected",
        { modelId: item.model.id, vendor, provider: item.providerName },
        LogScope.COMMAND,
      );
    } catch (error) {
