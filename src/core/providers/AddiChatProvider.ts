...
import { Model, ProviderRepository } from '../../common/types';
...
// Removed infrastructure imports: CustomToolManager, McpServerService
// because we now inject LLMService directly.
...
export class AddiChatProvider implements vscode.LanguageModelChatProvider {
  ...
  async provideLanguageModelChatResponse(
    ...
    const sid =
      typeof model.id === 'string' && model.id.startsWith('addi-provider:')
        ? model.id.replace('addi-provider:', '')
        : model.id;
    ...
    try {
      ...
      progress.report(
        new vscode.LanguageModelTextPart(
          `model query error: ${error instanceof Error ? error.message : 'unknown'}`
        )
      );
    } finally {
