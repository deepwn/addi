...
suite('ToolParser Unit Tests', () => {
  ...
  test('should determine visibility correctly', () => {
    const r1 = ToolParser.parse(
      { name: 't1', description: 'd', command: 'c' },
      't1.yaml',
      'global'
    );
    assert.strictEqual(r1?.visibility, 'global');

    const r2 = ToolParser.parse(
      { name: 't2', description: 'd', command: 'c' },
      't2.yaml',
      'workspace:public'
    );
    assert.strictEqual(r2?.visibility, 'public');

    const r3 = ToolParser.parse(
      { name: 't3', description: 'd', command: 'c' },
      't3.yaml',
      'workspace:private'
    );
