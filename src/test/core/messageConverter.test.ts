...
suite('MessageConverter Test Suite', () => {
    ...
    test('Drops orphan tool result', async () => {
        ...
        assert.strictEqual(coreMessages.length, 0, 'Should drop orphan tool result and result in empty messages if no other content');
    });
    ...
    test('Keeps mixed content (orphan tool dropped, text kept)', async () => {
        ...
        if (!msg) { throw new Error('msg missing'); }
