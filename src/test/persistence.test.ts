...
interface Params { temperature?:number; topP?:number; maxOutputTokens?:number; presencePenalty?:number; frequencyPenalty?:number; systemPrompt?:string | undefined }
...
function sanitizeParams(input: Params, prev: Params): Params {
  ...
  if(typeof input.topP === 'number') { out.topP = Math.min(Math.max(input.topP,0),1); }
  if(typeof input.maxOutputTokens === 'number') {
    const v = Math.floor(input.maxOutputTokens); if(isFinite(v)&&v>0) { out.maxOutputTokens = Math.min(Math.max(v,1),8192); }
  }
  if(typeof input.presencePenalty === 'number') { out.presencePenalty = Math.min(Math.max(input.presencePenalty,-2),2); }
  if(typeof input.frequencyPenalty === 'number') { out.frequencyPenalty = Math.min(Math.max(input.frequencyPenalty,-2),2); }
  if(typeof input.systemPrompt === 'string') { const sp = input.systemPrompt.trim(); out.systemPrompt = sp.length? sp: undefined; }
  return out;
}
